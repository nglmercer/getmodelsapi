...
export interface OpenRouterHTML {
  html: string;
  baseUrl: string;
}
...
export async function scrapeOpenRouter(config: ProviderConfig): Promise<ScraperResult> {
  try {
    ...
    const models = parseOpenRouterHTML(response.data);
    return {
      success: true,
      models,
    };
...
function parseOpenRouterHTML(html: string): Model[] {
...
export function parseOpenRouterPricing(pricingText: string): { prompt: number; completion: number } | undefined {
